...
//Clicar para selecionar o produto que queremos adicionar ao pedido
export function escolherPesquisaProdutoPromoPartidaPrest (selector) {
...
//Clicar para selecionar a voltagem que queremos adicionar ao pedido
export function escolherVoltagemProdutoPromoPartidaPrest (selector) {
...
//Botão adicionar produto após selecionar voltagem do produto
export function clicarAddPrdPromoPartidaPrest (selector) {
...
export function botaoGerarParcAlterVencPrest (selector) {

    cy.intercept('/views/carrinho/modalFormasPgto**').as('api_modal_formas_pagamento')

    //Botão "GERAR PARCELAS" - clicar
    cy.get('.gerar-parcelas > .layout-wrap > [style="padding: 0 5px"] > .md-primary')
        .click({force:true})
        
    cy.wait('@api_modal_formas_pagamento', { timeout: 40000 })
}